...
N = 5
WINDOW = 50
FINALPATH = 'your default final path'
# Viterbi algorithm
...
def viterbi(observations, **kwargs):
    ...
    filename = kwargs['filename'] if 'filename' in kwargs else FINALPATH
    ...
    for window_idx in range(len(observations) / window + 1):
        ...
        for t, obs in enumerate(current_obs):
            ...
            print('caochu', emission_probabilities)
            transition_probabilities = compute_transition_probabilities(previous_point, point, segments_table[t-1], segments)
    ...
    with open(observations_filename) as f:
        for i, line in enumerate(f):
            if i == 0:
                continue
            line = line.split(',')
            observations.append((float(line[0]), float(line[1])))
